      ...
      <div className="flex flex-col flex-1 px-4 md:px-8 py-6">
        ...
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {posts.map((post, index) => (
            <article
              key={post._id}
              className="relative isolate flex flex-col justify-end overflow-hidden rounded-xl px-6 pb-6 pt-40 shadow-md hover:shadow-xl transition-all duration-300"
            >
              <img
                src={post.images[currentImageIndex]}
                alt="Post Thumbnail"
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/40" />
              <h3 className="z-10 mt-2 text-xl font-semibold text-white">
                {post.hotelLocation}
              </h3>
              <div
                className="z-10 text-sm text-gray-200 cursor-pointer"
                onClick={() => navigate(`/admin/post/${post.slug}`)}
              >
                {post.title}
              </div>
